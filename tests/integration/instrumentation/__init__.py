...
def get_metrics_by_name(
    ...
    return prometheus_client.get_current_metric_value(
...
def _extract_exported_jobs(metrics):
    return {metric['metric']['exported_job'] for metric in metrics}
...
def get_histogram_rate_and_exported_jobs_by_name(
    ...
    if labels:
        label_list = [str(f"{key}='{labels[key]}'") for key in labels]
        query = (
            'rate('
            + metric_name
            + '_bucket'
